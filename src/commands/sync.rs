...
pub fn execute(args: SyncArgs) -> Result<()> {
    let pool = if let Some(jobs) = args.jobs.filter(|&j| j > 0) {
        Some(
    ...
    let main_logic = || -> Result<()> {
        ...
        let server_packages_dir = packages_dir
            ...
            .join("ServerPackages");
        let dev_packages_dir = packages_dir
            .parent()
            .unwrap_or_else(|| Path::new("."))
            .join("DevPackages");
        ...
        if all_missing.is_empty() {
            ...
            eprintln!();
            eprintln!("Hint: Try running `wally install` to fetch the missing dependencies");
